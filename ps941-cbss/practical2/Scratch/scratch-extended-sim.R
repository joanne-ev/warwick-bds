...
agent_matrix <- matrix(0, nrow = grid_size, ncol = grid_size)
agent_matrix

agent_pop <- agent_matrix %>% as.vector() %>% length() # population of agent matrix (i.e., grid_size^2)
agent_prop <- 0.1 # proportion of agent population to be activated
...
### ACTIVATING AGENTS ###
agent_matrix[loc] <- sample(1:2, samp, replace = TRUE, prob = c(0.4, 0.6)) # randomly activate agents 

# Count the number of (in)active agents (0 = inactive, 1 = normal, 2 = high-value)
sum(agent_matrix == 2) # high-value active
sum(agent_matrix == 1) # no-value active
sum(agent_matrix == 0) # inactive
...
for (i in seq(cycle)) {
  ...
  if (original_agent == 2) {
    if (activation_score > activation_threshold) {
      status <- "HV agent is activated"
      agent_matrix[agent_x, agent_y] <- 3
      colour_palette <- c("black", "orange", "green", "blue")
    } else {status <- "HV agent is not activated"}
  }
  
  # When a low-value agent gets activated
  if (original_agent == 1) {
    if (activation_score > activation_threshold) {
      status <- "LV agent is activated"
      agent_matrix[agent_x, agent_y] <- 1
    } else {status <- "LV agent is not activated" }
  }
